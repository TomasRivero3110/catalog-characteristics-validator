...
def Equipos(estado_opcion="activa"):
    ''' Funcion para consulta equipos'''
    global connection, listaEquipos
...
WHERE
     ...
     AND c.OFFR_CAT = 9142360187965786685
     ORDER BY a.PRD_OFF
 
        )x 
        """, estado=estado)
    for row in cursor:
        listaEquipos.append(list(row))
